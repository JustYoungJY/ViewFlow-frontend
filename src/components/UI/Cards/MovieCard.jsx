...
export default function MovieCard({movie}) {
    ...
    const limitedGenresString = Array.isArray(movie.genres) ? movie.genres.slice(0, 3).join(", ")
        : "";

    return (
        <Link to={`/movie/${movie.mediaId}`}
              className="block rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow">
            <img src={`https://image.tmdb.org/t/p/w500${movie.posterUrl}`} alt={movie.title} className="w-full h-64 object-cover"/>
